      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2 text-left mx-auto inline-block">
          {LOADING_PHASES.map((phase, idx) => (
            <div key={idx} className={`flex items-center space-x-3 text-sm transition-all duration-200 ${idx === loadingPhase ? 'text-blue-600 font-bold scale-[1.02]' : idx < loadingPhase ? 'text-green-500 opacity-60' : 'text-slate-300'}`}>
              <div className={`h-1.5 w-1.5 rounded-full ${idx === loadingPhase ? 'bg-blue-600 animate-pulse' : idx < loadingPhase ? 'bg-green-500' : 'bg-slate-200'}`}></div>
              <span>{phase}</span>
            </div>
          ))}
        </div>
